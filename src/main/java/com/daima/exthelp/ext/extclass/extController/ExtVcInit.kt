...
import com.daima.exthelp.Exp.SExp.Parser as SParser
...
import com.daima.exthelp.ext.extclass.extview.doubleBracketInsertHandler
...
class ExtVcInit(
    ...
    // 存储下级对象的数组
    private val subItems: MutableList<com.daima.exthelp.ext.extclass.extview.ExtViewItemClass> = mutableListOf()
    private val parse:Boolean=false;
    init {
        Info("init")
        // 初始化时尝试创建下级对象
        getInitFunc()
    ...
    fun getInitFunc() :ExtFunc?{
        jsObject.putUserData(CODE_HELP_KEY, this)
        //对对init函数跨处理
        val itemProperty = getObjProperty(jsObject, "init")
        if (itemProperty?.value !is JSFunction) {
            return null
        }
        val f=itemProperty.value as JSFunction
        if(f.block==null){
            return null
        }
        return ExtFunc(f,fun(): Context {
            return context
        })
    }
    // 获取所有子项
    fun getSubItems(): List<com.daima.exthelp.ext.extclass.extview.ExtViewItemClass> {
        return subItems
    }
    // 实现 CodeHelp 接口中的方法
    override fun reParse(psi:PsiElement):Boolean{
        //TODO
        val xtypeParser = SParser("LY{name xtype}")
        return true
    }
