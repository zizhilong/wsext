...
val namespaceToPsiElementMap: MutableMap<String, PsiElement> = mutableMapOf()
...
// 根据完全限定名称查找或创建 PsiElement 的函数
fun Class2Psi(name: String, project: Project): PsiElement? {
    loadNamespacePaths( project)
    // 检查 PsiElement 是否已在映射中
    ...
    // 将命名空间和路径转换为 PsiElement 的逻辑
    val newPsiElement = findPsiElementForNamespace(baseDir, remainingPath, project) ?: return null

    // 将结果存储在映射中以供将来使用
    namespaceToPsiElementMap[name] = newPsiElement

    return newPsiElement
}

// 用于根据基础目录和路径查找 PsiElement 的函数
fun findPsiElementForNamespace(baseDir: String, path: String, project: Project): PsiElement? {
    // 获取项目根目录
    ...
    // 使用 PsiManager 从 VirtualFile 获取 PsiElement
